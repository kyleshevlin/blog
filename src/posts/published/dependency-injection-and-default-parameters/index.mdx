...
This is a simple, well named function. It _formats_ a string. But the part I want to focus on for a moment is use of `capitalize`. What is `capitalize` in this situation?
...
In the case of primitive values, such as `Number`s, `String`s and `Boolean`s, the criteria is can be fairly simple: use the item itself. Let's start building this `counts` function, but with a `for` loop. Don't want to spoil all my course material:
