...
Recursion is the pattern of a function calling itself. A very common example of a recursive function is calculating a _factorial_ of a number. <Marker index={1} content="A factorial is a positive number multiplied by every number between itself and 1. So <code>6!</code> is <code>6 * 5 * 4 * 3 * 2 * 1</code>." /> Consider the following function:
...
Easy, right?! <Marker index={2} content="It is. It just takes practice." />
...
Similar to how we recognized that `factorial` can be solved by using the result of `factorial` called on smaller numbers, we can recognize that a tree is the result of combining smaller trees. <Marker index={3} content="One might think of it as a fractal." /> Understanding this, we can make a `Tree` component that will call itself to render smaller `Tree`s.
