...
What happens if the designer requests a third button? Do we add an `actionMiddle` prop? What happens if the user provides `actionMiddle` and `actionRight`, but know `actionLeft`, does `actionMiddle` _become_ an `actionLeft`, or should there be an empty space to the left?
