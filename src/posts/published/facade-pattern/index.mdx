...
description: 'The Facade Pattern is when we wrap a library or object in our own function or class in order to hide the implementation details from the consumer. We can hide vast complexity and build in flexibility with this pattern.'
...
We live in exciting times. There's almost a library for everything you can imagine on [npm](https://npmjs.com) and you can add packages to your projects so quickly. It's really quite a bit of fun when you take a second to think about it.

But this ability to quickly grab and use packages hides some technical debt you _might_ be creating when you do so. There's a reason these packages are called "dependencies", and if you're not careful, you can wind up dependent upon something that is terribly difficult to manage or remove later.

I want to introduce you to a pattern of programming called the "facade pattern". I get the term from the "Gang of Four" (aka GoF) book [Design Patterns](https://www.amazon.com/Design-Patterns-Elements-Reusable-Object-Oriented/dp/0201633612?dchild=1&keywords=design+patterns&qid=1613630581&sr=8-4&linkCode=ll1&tag=kyleshevlin-20&linkId=caa9cd1b6f46468d77c7b2e6d3ac3650&language=en_US&ref_=as_li_ss_tl) if you'd like to do additional reading.

Essentially, the facade pattern is this: we add a layer of abstraction between the consumer of some code and the implementation of that code. In regards to packages like I'm describing above, it's creating a wrapping function or class with that uses the package. There are at least 2 reasons to do this:
...
On the other hand, libraries for fetching data or date formatting can make good candidates for the facade pattern. Let's look at fetching data as an example.
...
If another great library for fetching data comes along, we can update the implementation of `API` with out needing to change any of our components that use it.

### Additional Thoughts

Using this pattern on key parts of your project might save you from a massive overhaul in a few years. Fetching data isn't the only thing it's good for. For example, right now your project might use the `moment` library, but you'd like to try `date-fns` instead. A facade may make that possible. It will certainly make it easy to switch to `Temporal` when it lands. <Marker content={`<code>Temporal</code> is a new datetime object coming to JavaScript. You can read more here: <a href="https://github.com/tc39/proposal-temporal">https://github.com/tc39/proposal-temporal</a>`} />
