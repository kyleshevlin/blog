...
One of the most common misunderstandings I run into with people regarding React is this: If a component has a state change and rerenders, it _does not_ cause `children` to rerender. I think this is very obvious when we _remove_ JSX, and use `React.createChildren` instead. Let's have a look.

Here we have some code that has a state change, but also renders `children`. We'll show those state changes by using a `Box` that changes its background color with every render.
