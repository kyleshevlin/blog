...
Our compositions now read left-to-right, top-to-bottom. <Marker index={4} content="This is also how the pipeline operator <code>|></code> works in other languages" /> Forgive me, I had to teach you composition the mathematical way first. Our `pipe` compositions now would look like this:

```javascript
const withExcitement = pipe(
  scream,
  exclaim
)
