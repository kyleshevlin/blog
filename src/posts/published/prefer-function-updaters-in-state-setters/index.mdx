...
excerpt: 'React state can be updated in two ways: replace the `currentState` with the `nextState`, or transform the `currentState` to the `nextState` with a function. Prefer the function.'
...
This "gotcha" has also been true since the class component days. If you call a state setter more than once in the same function body, there's a _really_ good chance that the update will not be what you expect when you that body completes.
