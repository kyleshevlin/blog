...
The very first error we see is: `Parameter 'input' implicitly has an 'any' type`. We can knock this one out easily. We don't want to specify a type of `input`, we simply want whatever the user passes in to be the type throughout the function. This is exactly what a generic is for, so let's add one:
...
Next, we're going to make our factory stateless. We're going to pass in the "state" of the `api` and the initial states are `isMatched` set to `false` and `result` set to `undefined`, like so:
...
I hope it's clear, I'm _not_ arguing we should stop using TypeScript. But I am arguing that we should be mature enough to acknowledge how challenging it can be to write type safe code in it. That sometimes we give up having some "good things" for the sake of avoiding many "bad things".
