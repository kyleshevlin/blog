...
That it is to do the object equivalent:
...
The return type is a non-fixed-length heterogenous array of the union of `boolean` and the shape of our `handlers` object. We'll run into a lot of annoying type issues if we try and use this in a component.
...
As we've seen, it thinks its an array and this is understandable because we don't have _real_ tuples. So taking our `useBool` function, we could add a return type to it.
